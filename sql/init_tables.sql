...
-- 删除现有表（如果存在）
DROP TABLE IF EXISTS staff_organization CASCADE;
...
COMMENT ON TABLE staff IS '教职工表';
...
-- 删除已存在的策略
DROP POLICY IF EXISTS "允许所有用户查看组织机构" ON organization;
DROP POLICY IF EXISTS "允许所有用户查看教职工基本信息" ON staff;
DROP POLICY IF EXISTS "允许所有用户查看教职工任职信息" ON staff_organization;
...
CREATE INDEX IF NOT EXISTS idx_organization_type ON organization(org_type);
CREATE INDEX IF NOT EXISTS idx_organization_level ON organization(level_num);
...
CREATE INDEX IF NOT EXISTS idx_staff_organization_staff_id ON staff_organization(staff_id);
CREATE INDEX IF NOT EXISTS idx_staff_organization_org_id ON staff_organization(org_id); 